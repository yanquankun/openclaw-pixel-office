...
本项目基于 [pixel-agents](https://github.com/pablodelucca/pixel-agents) (MIT License) 二次开发，遵循以下规范：
...
在开发 OpenClaw 多 Agent 协作系统时，我们面临一个痛点：

> **"7 个 Bot 在后台忙碌，但用户看不到它们在干什么"**

传统的日志/终端输出太抽象，不够直观。所以我们做了 **Pixel Office**：

- 🎮 **游戏化可视化**：用像素小人实时展示每个 Bot 的状态
- 🏢 **场景化呈现**：休息区、工作区、BUG 区，不同状态去不同区域
- 📊 **实时反馈**：接到任务 → 走到工位 → 开始工作 → 完成任务，全流程可见
- 🎨 **情感化设计**：像素风格 + 可爱动画，让监控变得有趣

这不仅仅是一个监控面板，更是**一人公司（OPC）的虚拟办公室** —— 你的每个 AI 员工都有工位、会走动、会休息。
